...
# --- SAYFA AYARLARI ---
st.set_page_config(
    page_title="Patronun Terminali v4.4.0 (ICT Pro)",
    layout="wide",
    page_icon="🐂"
)
...
<style>
    ...
    /* ICT BAR STİLİ */
    .ict-bar-container {
        width: 100%; height: 8px; background-color: #e2e8f0; border-radius: 4px; overflow: hidden; margin: 4px 0; display:flex;
    }
    .ict-bar-fill { height: 100%; }
    .ict-badge { display:inline-block; padding:1px 4px; border-radius:3px; font-size:0.6rem; font-weight:700; margin-right:3px; }
...
# --- ICT GELISTIRILMIS (ENHANCED - PRICE ACTION & OTE) ---
@st.cache_data(ttl=600)
def calculate_ict_concepts(ticker):
    try:
        ...
        # 1. SWING FRACTAL TESPİTİ (5 Barlı Fractal: 2 sol - 1 orta - 2 sağ)
        # Basit bir loop ile son belirgin Swing High/Low bulalım
        sw_highs = []
        sw_lows = []
        ...
        # 2. MARKET YAPISI (BOS vs MSS)
        structure = "YATAY"
        bias_color = "gray"
        
        # Son swing noktalarını ihlal ettik mi?
        if curr_price > last_sh:
            structure = "BULLISH BOS (Trend Devamı)"
            bias_color = "green"
        elif curr_price < last_sl:
            structure = "BEARISH BOS (Trend Devamı)"
            bias_color = "red"
        else:
            # Range içindeyiz, internal yapıya bakalım
            if last_sh < prev_sh and last_sl > prev_sl:
                structure = "KONSOLİDASYON (Sıkışma)"
                bias_color = "yellow"
            elif curr_price > (last_sh + last_sl) / 2:
                 structure = "INTERNAL BULLISH"
                 bias_color = "blue"
            else:
                 structure = "INTERNAL BEARISH"
                 bias_color = "blue"

        # 3. DEALING RANGE & FIBONACCI (OTE)
        # En son belirgin range'i bulmak için son Swing High ve Swing Low arasını al
        # Eğer Bullish isek, son Low'dan High'a çekeriz.
        range_high = max(last_sh, curr_price)
        range_low = min(last_sl, curr_price)
        
        # Basitleştirilmiş: Son anlamlı iki fractal arası range
        if sw_highs[-1][2] > sw_lows[-1][2]: # High daha güncel -> Yükseliş dalgası (Low -> High)
            r_low = sw_lows[-1][1]
            r_high = sw_highs[-1][1]
            mode = "UP"
        else: # Low daha güncel -> Düşüş dalgası (High -> Low)
            r_low = sw_lows[-1][1]
            r_high = sw_highs[-1][1]
            mode = "DOWN"
        ...
        if range_pos_pct > 50:
            pos_label = "PREMIUM (Satış Bölgesi)"
            is_discount = False
            if 62 <= range_pos_pct <= 79: is_ote = True # Short OTE
        else:
            pos_label = "DISCOUNT (Alış Bölgesi)"
            is_discount = True
            if 21 <= range_pos_pct <= 38: is_ote = True # Long OTE (100-79=21, 100-62=38 ters mantık)

        # 4. FVG (FAIR VALUE GAP) TESPİTİ
        # Son 30 mumda, mitigation olmamış (doldurulmamış) en yakın FVG
        bullish_fvg = []
        bearish_fvg = []
        ...
        for i in range(len(df)-2, len(df)-30, -1):
            ...
            if low.iloc[i] > high.iloc[i-2]:
                ...
                if curr_price > gap_bot: # Fiyat hala üzerinde veya içinde
                    bullish_fvg.append((gap_bot, gap_top))
            
            # Bearish FVG: Mum(i) High < Mum(i-2) Low
            if high.iloc[i] < low.iloc[i-2]:
                gap_top = low.iloc[i-2]
                gap_bot = high.iloc[i]
                if curr_price < gap_top: # Fiyat hala altında veya içinde
                    bearish_fvg.append((gap_bot, gap_top))
        ...
        if is_discount and bullish_fvg:
            # Discount bölgesindeysek Bullish FVG ararız
            bg = bullish_fvg[0] # En güncel
            active_fvg = f"BISI: {bg[0]:.2f} - {bg[1]:.2f}"
            fvg_color = "green"
        elif not is_discount and bearish_fvg:
            # Premium bölgesindeysek Bearish FVG ararız
            bg = bearish_fvg[0]
        ...
        # Fiyatın üzerindeki en yakın Swing High -> BSL
        # Fiyatın altındaki en yakın Swing Low -> SSL
        ...
        liq_target = "-"
        if structure.startswith("BULLISH") and next_bsl:
            liq_target = f"BSL Hedefi: {next_bsl:.2f}"
        elif structure.startswith("BEARISH") and next_ssl:
            liq_target = f"SSL Hedefi: {next_ssl:.2f}"
        else:
            ...
            if dist_bsl < dist_ssl:
                liq_target = f"En Yakın: {next_bsl:.2f} (High)"
            else:
                liq_target = f"En Yakın: {next_ssl:.2f} (Low)"

        # 6. GOLDEN SETUP KONTROLÜ
        golden_txt = "Beklemede"
        is_golden = False
        
        # Bullish Golden: Discount Bölgesi + Bullish Structure + Yakında FVG var
        if is_discount and "BULLISH" in structure and bullish_fvg:
            golden_txt = "LONG FIRSATI (Trend+Ucuz+FVG)"
            is_golden = True
        # Bearish Golden: Premium Bölgesi + Bearish Structure + Yakında FVG var
        elif not is_discount and "BEARISH" in structure and bearish_fvg:
...
def render_ict_panel(analysis):
    ...
    # Progress Bar Hesabı
    bar_width = min(max(pos_pct, 5), 95) # Görsel taşmaması için
    
    golden_badge = ""
    if analysis['is_golden']:
        golden_badge = f"<div style='margin-top:4px; background:#fef9c3; border:1px solid #fde047; color:#854d0e; padding:4px; border-radius:4px; font-weight:bold; text-align:center;'>✨ {analysis['golden_text']}</div>"
    elif analysis['ote_level']:
        golden_badge = f"<div style='margin-top:4px; background:#eff6ff; border:1px solid #bfdbfe; color:#1e40af; padding:4px; border-radius:4px; text-align:center;'>🎯 OTE Bölgesi</div>"
    ...
    <div class="info-card">
        ...
        <div style="margin: 6px 0;">
            ...
            <div class="ict-bar-container">
                <div class="ict-bar-fill" style="width:{bar_width}%; background: linear-gradient(90deg, #4ade80 0%, #3b82f6 50%, #f87171 100%);"></div>
            </div>
            <div style="text-align:center; font-size:0.7rem; font-weight:600; color:#0f172a;">
                {analysis['pos_label']} <span style="color:#64748B; font-size:0.6rem;">(%{pos_pct:.1f})</span>
            </div>
            <div style="text-align:center; font-size:0.6rem; color:#94a3b8;">
        ...
        <div style="margin-top:8px;">
            <div class="info-row">
                ...
                <div class="info-val" style="color:{'#166534' if analysis['fvg_color']=='green' else '#991b1b'}; font-weight:600;">{analysis['fvg']}</div>
            </div>
            <div class="info-row">
                <div class="label-long">Likidite (Hedef):</div>
...
<div class="header-container" style="display:flex; align-items:center;">
    ...
    <div>
        ...
        <div style="font-size:0.8rem; color:#64748B;">Market Maker Edition (ICT Enhanced)</div>
    </div>
</div>
...
with col_left:
    ...
    # ICT Panel artık burada değil, sağ tarafta
    # render_ict_panel(...) sağ tarafa taşındı
