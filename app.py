...
def radar2_scan(asset_list, min_price=5, max_price=500, min_avg_vol_m=1.0):
    ...
    # 1. ADIM: ANA HİSSE VERİSİNİ İNDİR (BURASI KRİTİK)
    try:
        data = yf.download(asset_list, period="1y", group_by='ticker', threads=True, progress=False)
    except:
        return pd.DataFrame() # Sadece ana veri yoksa dur.

    # 2. ADIM: ENDEKS VERİSİNİ İNDİR (BURASI OPSİYONEL - HATA OLSA DA DEVAM ET)
    spy_ret = 0.0
    try:
        spy_data = yf.download("^GSPC", period="1y", progress=False)['Close']
        if not spy_data.empty:
            spy_ret = spy_data.pct_change(60).iloc[-1]
    except:
        spy_ret = 0.0 # Endeks inmezse RS'i 0 kabul et, YOLA DEVAM ET.
    ...
    # Filtreler (NaN -> False)
    price_filter = ((c >= min_price) & (c <= max_price)).fillna(False)
    vol_filter = (avg_vol_20.iloc[-1] >= min_avg_vol_m * 1e6).fillna(False)
    
    # Trend
    bullish = (c > sma50.iloc[-1]) & (sma50.iloc[-1] > sma100.iloc[-1]) & \
              (sma100.iloc[-1] > sma200.iloc[-1]) & \
              (sma200.iloc[-1] > sma200.iloc[-20])
    bullish = bullish.fillna(False)
    
    bearish = (c < sma200.iloc[-1]) & (sma200.iloc[-1] < sma200.iloc[-20])
    bearish = bearish.fillna(False)

    scores = pd.Series(0, index=close.columns)
    tags = pd.Series("", index=close.columns)
    setups = pd.Series("-", index=close.columns)
    
    # Setup 1: Breakout
    recent_high_60 = high.rolling(60).max().iloc[-1]
    is_breakout = bullish & (c >= recent_high_60 * 0.97)
    
    # Setup 2: Pullback
    is_pullback = bullish & (c >= sma20.iloc[-1]) & (c <= sma50.iloc[-1]*1.02) & \
                  (rsi.iloc[-1] >= 40) & (rsi.iloc[-1] <= 55)
                  
    # Setup 3: Reversal
    is_reversal = (rsi.shift(1).iloc[-1] < 30) & (rsi.iloc[-1] >= 30) & \
                  (macd_hist.iloc[-1] > macd_hist.shift(1).iloc[-1])

    # Puanlama
    scores[is_breakout] += 2; tags[is_breakout] += "Zirve | "; setups[is_breakout] = "Breakout"
    scores[is_pullback] += 2; tags[is_pullback] += "Düzeltme | "; setups[is_pullback] = "Pullback"
    scores[is_reversal] += 2; tags[is_reversal] += "Dip Dönüşü | "; setups[is_reversal] = "Dip Dönüşü"
    
    vol_spike = (volume.iloc[-1] > avg_vol_20.iloc[-1] * 1.3).fillna(False)
    scores[vol_spike] += 1; tags[vol_spike] += "Hacim+ | "
    
    rs_positive = (rs_score.iloc[-1] > 0).fillna(False)
    scores[rs_positive] += 1; tags[rs_positive] += "RS+ | "
    
    scores[bullish] += 1
    scores[bearish] -= 1
    
    trend_str = pd.Series("Yatay", index=close.columns)
    trend_str[bullish] = "Boğa"
    trend_str[bearish] = "Ayı"

    # Sonuç Filtreleme
    valid_indices = vol_filter & price_filter & (scores > 0)
    
    results = pd.DataFrame({
        "Sembol": scores.index,
        "Fiyat": c.values,
        "Trend": trend_str.values,
        "Setup": setups.values,
        "Skor": scores.values,
        "RS": rs_score.iloc[-1].values,
        "Etiketler": tags.values
    })
    
    results = results[valid_indices.values]
    results["Fiyat"] = results["Fiyat"].round(2)
    results["RS"] = results["RS"].round(1)
    results["Etiketler"] = results["Etiketler"].str.rstrip(" | ")
    
    return results.sort_values(by=["Skor", "RS"], ascending=False).head(50)
