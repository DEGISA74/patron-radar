...
# --- SAYFA AYARLARI ---
st.set_page_config(
    page_title="Patronun Terminali v4.1.2",
    layout="wide",
    page_icon="🐂"
)
...
def calculate_ict_concepts(ticker):
    try:
        ...
        return {
            ...
            "fvg": fvg_text, "ob": ob_text, "ob_label": "Bullish Order Block (OB)",
            "liquidity": liq_text, "liq_label": "Liquidity Pool", "fibo": fibo_text,
            "raw_ote": ote_bull if is_bullish else ote_bear
        }
    except:
        return None
...
with col_left:
    ...
    st.write("")
    # Değişiklik 1: Grafik yüksekliği 650px olarak ayarlandı
...
with col_right:
    ...
    with tab1:
        ...
        if st.session_state.scan_data is not None and not st.session_state.scan_data.empty:
            # Değişiklik 2: Scrollbar (500px yükseklik)
...
# --- SIDEBAR (Değişiklik 3: AI Prompt) ---
with st.sidebar:
    ...
    with st.expander("🤖 AI Analist (Prompt Oluştur)", expanded=True):
        ...
        if st.button("📋 Analiz Metnini Hazırla", type="primary"):
            ...
            # Verileri çekip hazırlıyoruz
            inf = fetch_stock_info(t); price = inf['price'] if inf else "Bilinmiyor"
            ict = calculate_ict_concepts(t); tech = get_tech_card_data(t)
            ...
            prompt_text = f"""Rol: Profesyonel borsa traderı.
...
ICT Seviyeleri:
- Konum: {ict['position'] if ict else '-'}
- Destek (OB): {ict['ob'] if ict else '-'}
- Fırsat (FVG): {ict['fvg'] if ict else '-'}
- OTE (Fib %62): {ict['raw_ote'] if ict else '-'}
