...
def get_sentiment(text):
    """Basit Duygu Analizi (İleride burayı GPT ile değiştireceğiz)"""
    blob = TextBlob(text)
    score = blob.sentiment.polarity
    if score > 0.1: return "YUKARI (Bullish)", "🟢", score
    elif score < -0.1: return "AŞAĞI (Bearish)", "🔴", score
    else: return "NÖTR", "⚪", score

def determine_source_level(publisher):
    """Haber kaynağını Piramit Modeline göre derecelendirir"""
...
# --- ARAYÜZ ---
st.title("🦅 Patronun Piyasa Radarı v0.1")
st.markdown("*Akılcı Şüphe, Tam Doğrulama*")
st.divider()

# Sidebar
with st.sidebar:
    st.header("Kontrol Paneli")
    ticker = st.text_input("Hisse Kodu (Örn: AAPL, THYAO.IS)", value="AAPL").upper()
    st.info("Level 1 kaynaklar önceliklendirilmiştir.")
...
if ticker:
    ...
    if price:
        ...
        c3.metric("F/K Oranı", info.get('trailingPE', '-'))
        ...
        with col_news:
            st.subheader("📡 Haber Analiz Akışı")
            if news_df is not None and not news_df.empty:
                for _, row in news_df.iterrows():
                    # Stil belirleme
                    css_class = "bullish" if "YUKARI" in row['Yön'] else "bearish" if "AŞAĞI" in row['Yön'] else "neutral"
                    ...
                    <div class="metric-card {css_class}" style="margin-bottom:10px;">
                        ...
                        <h5><a href="{row['Link']}" style="color:white;text-decoration:none;">{row['Başlık']}</a></h5>
                        <p>{row['Yön']}</p>
                    </div>
                    """, unsafe_allow_html=True)
            else:
                st.warning("Haber akışı yok.")
                
        with col_chart:
            st.subheader("📈 Fiyat Grafiği (Price Action)")
            st.line_chart(yf.Ticker(ticker).history(period="1y")['Close'])
            st.caption("Yakında buraya ICT konseptleri ve FVG kutuları eklenecek.")
