...
# --- SAYFA AYARLARI ---
st.set_page_config(page_title="Patronun Terminali v3.5.3 (Sıkı Filtre)", layout="wide", page_icon="🦅")
...
def analyze_market_intelligence(asset_list):
    ...
    for symbol in asset_list:
        try:
            ...
            if score >= 3: # Eşik tekrar 3'e yükseltildi (Daha güvenilir sinyal)
                signals.append({"Sembol": symbol, "Fiyat": f"{curr_c:.2f}", "Skor": score, "Nedenler": " | ".join(reasons)})

        except Exception: continue
    
    if not signals: return pd.DataFrame()
    return pd.DataFrame(signals).sort_values(by="Skor", ascending=False).head(20) # Top 20 limiti korundu
...
# --- ARAYÜZ (KOKPİT) ---
st.title(f"🦅 Patronun Terminali v3.5.3")
st.markdown("---")
