                    ...
                    if max_green_recent > (avg_body.iloc[-1] * 1.5) and vol_check:
                        ...
                        for i in range(n - 1, max(n - 6, 2), -1):
                            if low_arr[i] > high_arr[i - 2]:           # Bullish FVG
                                ...
                                # Mitigated: sonraki herhangi bir low gap'e girdi mi?
                                if any(low_arr[j] < fvg_top for j in range(i + 1, n)):
                                    continue                             # Dolmuş FVG — atla
                                # Consequent Encroachment (CE): FVG'nin %50 orta noktası
...
def compile_confluence_hits():
    ...
    3 bağımsız metodoloji grubunda her birinde kaç grupta çıktığını sayar.
      Grup 1 — Yapısal : ICT, Royal Flush
...
# ==============================================================================
# 🔮 HARMONİK FORMASYON MOTORU (Fibonacci XABCD)
# ==============================================================================
...
def _main_price_chart_b64(symbol, dark_mode):
    ...
    try:
        ...
        bg   = '#0b0f19' if dark_mode else '#ffffff'
        fg   = '#e2e8f0' if dark_mode else '#1e293b'
        grid = '#1e2d40' if dark_mode else '#f1f5f9'
        c_up   = "#228dbb"
        c_down = "#d36664"
...
def _main_price_chart_plotly(symbol, dark_mode):
    ...
    try:
        ...
        bg   = '#0b0f19' if dark_mode else '#ffffff'
        fg   = '#e2e8f0' if dark_mode else '#1e293b'
        grid = 'rgba(255,255,255,0.06)' if dark_mode else 'rgba(0,0,0,0.06)'
        paper_bg = '#0b0f19' if dark_mode else '#ffffff'
...
def calculate_8_point_roadmap(ticker):
    ...
    try:
        ...
        if _ema_bull_now:
            ...
            _ema_align_html = (f'<b>EMA Hizalaması:</b> <span style="color:#dc2626;font-weight:700;">▼ Aşağı Hizalı</span>'
                               f'<span style="color:#64748b;font-size:0.7em;"> ({_ema_streak} gün)</span>')
        else:
...
def render_roadmap_8_panel(ticker):
    ...
    _STATUS_CFG = {
        "bull":    ("22, 163, 74",   "#16a34a"),
        "bear":    ("239, 68, 68",   "#dc2626"),
        "warning": ("245, 158, 11",  "#d97706"),
...
def render_unified_signals_panel(ticker):
    ...
    try:
        ...
        if master_score >= 70:
            ...
            karar_color = "#10b981" if is_dark else "#15803d"
            panel_border = "#16a34a"
            panel_bg = "rgba(5,46,22,0.45)" if is_dark else "#f0fdf4"
            title_col = "#4ade80" if is_dark else "#15803d"
        elif master_score >= 45:
            karar_icon, karar_txt = "🟡", "İZLE"
            karar_color = "#f59e0b" if is_dark else "#d97706"
            panel_border = "#d97706"
            panel_bg = "rgba(69,26,3,0.45)" if is_dark else "#fffbeb"
            title_col = "#fbbf24" if is_dark else "#92400e"
        else:
            karar_icon, karar_txt = "🔴", "UZAK DUR"
            karar_color = "#ef4444" if is_dark else "#dc2626"
            panel_border = "#dc2626"
            panel_bg = "rgba(69,10,10,0.45)" if is_dark else "#fef2f2"
            title_col = "#f87171" if is_dark else "#991b1b"

        border_dim = "rgba(255,255,255,0.06)" if is_dark else f"{panel_border}30"
        ...
        try: lor = calculate_lorentzian_classification(ticker)
        except: pass
        ...
        try:
            ...
            if stp:
                if stp['type'] == 'cross_up':
                    signals.append(("⚡","STP Yukarı Kesişim","#10b981" if is_dark else "#15803d","Kısa vadeli alıcılar iştahlandı. Fiyat denge noktasını yukarı kırdı, taze bir yükseliş ivmesi tetiklendi.",True))
                elif stp['type'] == 'cross_down':
                    signals.append(("🔻","STP Aşağı Kesişim","#ef4444" if is_dark else "#dc2626","Kısa vadeli satıcı baskısı taze. Denge noktası aşağı kırıldı — likidite çıkışı başladı.",False))
                elif stp['type'] == 'trend_up':
                    g = stp['data'].get('Gun','?')
                    signals.append(("📈",f"STP Yükseliş Trendi ({g} gün)","#38bdf8" if is_dark else "#0369a1",f"Alıcılar {g} gündür kontrolde. Trend devam ettiği sürece dip alımları geçerli strateji.",True))
                elif stp['type'] == 'trend_down':
                    g = stp['data'].get('Gun','?')
                    signals.append(("📉",f"STP Düşüş Trendi ({g} gün)","#f87171" if is_dark else "#b91c1c",f"Satıcılar {g} gündür baskıda. Tepki rallileri kısa, ana trend aşağı.",False))
        except: pass
        ...
        try:
            if lor and lor['votes'] >= 7:
                is_up = lor['signal'] == "YÜKSELİŞ"
                lc = ("#10b981" if is_dark else "#15803d") if is_up else ("#ef4444" if is_dark else "#dc2626")
                signals.append(("🧠",f"Lorentzian: {lor['signal']} %{int(lor['prob'])}",lc,f"ML modeli son 2000 günün en benzer hareketlerini taradı. %{int(lor['prob'])} ihtimalle {'yukarı' if is_up else 'aşağı'} yön sinyali.",is_up))
        except: pass
        ...
        try:
            if ict_data and sent_data:
                ...
                c2 = "DISCOUNT" in ict_data.get('zone','') or "MSS" in ict_data.get('structure','') or "BOS" in ict_data.get('structure','')
                c3 = "Güçlü" in ict_data.get('displacement','') or "Hacim" in sent_data.get('vol','') or sent_data.get('raw_rsi',0)>55
                if c1 and c2 and c3:
                    signals.append(("🏆","Altın Fırsat (Güç+Konum+Enerji)","#ca8a04" if is_dark else "#a16207","3 bağımsız koşul aynı anda: RS güçlü, fiyat DISCOUNT bölgesinde, hacim momentum destekliyor.",True))
        except: pass

        # ── 7. Royal Flush (4/4 Kraliyet) ───────────────────────────
        try:
        ...
        try:
            ...
            if hc:
                ...
                signals.append(("⚡",f"Harmonik Confluence: {hc['pattern']} + ICT {hc['zone']} | PRZ:{hc['prz']:.2f}","#a78bfa" if is_dark else "#6d28d9","Fibonacci yapısı, ICT bölgesi ve RSI diverjansının aynı noktada çakışması. Üç metodoloji aynı dönüş seviyesini işaret ediyor.",is_hcp))
        except: pass
        ...
        try:
            ...
            if acc:
                ...
                signals.append(("⚡" if is_pp else "🤫","Akıllı Para: Pocket Pivot (Patlama)" if is_pp else "Akıllı Para: Sessiz Toplama","#c084fc" if is_dark else "#7c3aed","Pocket Pivot: Büyük hacimle fiyat yükseldi — kurumsal alım izleri." if is_pp else "Fiyat yatay veya baskılı görünse de arka planda sinsi fon alımı var. Kırılım hazırlığı.",True))
        except: pass
        ...
        try:
            if pa:
                div_type = pa.get('div',{}).get('type','neutral')
                if div_type == 'bullish':
                    signals.append(("💎","RSI Uyumsuzluk: POZİTİF (Gizli Güç)","#10b981" if is_dark else "#15803d","Fiyat yeni dip yaparken RSI yüksek dip yapıyor. Satıcıların gücü azalıyor — büyükler dipten topluyor olabilir.",True))
                elif div_type == 'bearish':
                    signals.append(("🐻","RSI Uyumsuzluk: NEGATİF (Yorgun Boğa)","#ef4444" if is_dark else "#b91c1c","Fiyat yeni zirve yaparken RSI düşük zirve yapıyor. Yükseliş devam ediyor gibi görünse de içten çürüyor.",False))
                sv = pa.get('smart_volume',{})
                if sv.get('stopping','Yok') != 'Yok':
                    signals.append(("🐋","Balina İzi: Stopping Volume","#34d399" if is_dark else "#15803d","Düşüş yüksek hacimle karşılandı. Kurumsal fren devrede, düşüş durduruluyor olabilir.",True))
                if sv.get('climax','Yok') != 'Yok':
                    signals.append(("🌋","Balina İzi: Climax Volume","#f97316" if is_dark else "#ea580c","Rallinin zirvesinde anormal hacim. Akıllı para malı küçük yatırımcıya boşaltıyor olabilir.",False))
        except: pass

        # ── 19. Z-Score ──────────────────────────────────────────────
        try:
            z = calculate_z_score_live(df)
            if z <= -2.0:
                signals.append(("🔥",f"İstatistiksel DİP (Z-Score: {z:.2f})","#10b981" if is_dark else "#059669","Fiyat ortalamadan -2 standart sapma altında. Aşırı satım bölgesi — istatistiksel dönüş ihtimali yüksek.",True))
            elif z <= -1.5:
                signals.append(("⚠️",f"Dibe Yaklaşıyor (Z-Score: {z:.2f})","#fbbf24" if is_dark else "#d97706","Fiyat ortalamanın 1.5 std sapma altında. Potansiyel dip bölgesi yakınında.",True))
            elif z >= 2.0:
                signals.append(("🚨",f"İstatistiksel TEPE (Z-Score: {z:.2f})","#ef4444" if is_dark else "#dc2626","Fiyat ortalamadan +2 standart sapma üzerinde. Matematiksel aşırı fiyatlanma — düzeltme riski yüksek.",False))
            elif z >= 1.5:
                signals.append(("⚠️",f"Tepeye Yaklaşıyor (Z-Score: {z:.2f})","#f97316" if is_dark else "#ea580c","Fiyat ortalamanın 1.5 std sapma üzerinde. Düzeltme riski artıyor.",False))
            elif z >= 1.0:
                signals.append(("📈",f"Pahalılanıyor (Z-Score: {z:.2f})","#eab308" if is_dark else "#854d0e","Fiyat ortalamanın 1 std sapma üzerinde. Yeni alımlarda temkinli ol.",False))
        except: pass
        ...
        try:
            ...
            if hm:
                ...
                hmc = ("#34d399" if is_dark else "#15803d") if is_hmp else ("#f87171" if is_dark else "#b91c1c")
        ...
        # ── Ayır & Render ────────────────────────────────────────────
        pos_sigs = [(i,t,c,e) for i,t,c,e,p in signals if p]
        neg_sigs = [(i,t,c,e) for i,t,c,e,p in signals if not p]

        if not signals:
            return

        edu_col = "#f1f5f9" if is_dark else "#1e293b"
        def _row(icon, text, color, edu, is_pos):
            ...
            return (
                ...
                f"<span style='font-size:0.82rem;font-weight:600;line-height:1.3;flex:1;color:{color};'>{text}</span>"
        ...
        pos_html = "".join(_row(i,t,c,e,True)  for i,t,c,e in pos_sigs) or f"<div style='font-size:0.75rem;color:#64748b;font-style:italic;padding:4px 2px;'>Aktif olumlu sinyal yok.</div>"
...
def _render_health_signals_panel():
    ...
    try:
        if "ticker" in st.session_state and st.session_state.ticker:
            ...
            pos_items_html = ""
            if score_pros:
                _p_txt  = "#a7f3d0" if _dark else "#14532d"
                _p_line = "rgba(16,185,129,0.15)" if _dark else "rgba(22,163,74,0.15)"
                for p in score_pros:
                    pos_items_html += (
                        f"<div style='display:flex;align-items:flex-start;gap:6px;"
                        f"padding:5px 0;border-bottom:1px solid {_p_line};'>"
                        f"<span style='color:#10b981;font-size:0.65rem;margin-top:2px;flex-shrink:0;'>●</span>"
                        f"<span style='font-size:0.7rem;line-height:1.4;color:{_p_txt};'>{p}</span>"
                        f"</div>"
                    )
            else:
                _p_txt = "#6ee7b7" if _dark else "#6b7280"
            ...
            if _dark:
                _pos_hdr_bg   = "linear-gradient(90deg,#052e16 0%,#0d2818 100%)"
                _pos_hdr_txt  = "#6ee7b7"
                _pos_cnt_bg   = "rgba(5,46,22,0.55)"
                _pos_border   = "#10b981"
            else:
                _pos_hdr_bg   = "linear-gradient(90deg,#dcfce7 0%,#f0fdf4 100%)"
                _pos_hdr_txt  = "#166534"
                _pos_cnt_bg   = "#f0fdf4"
                _pos_border   = "#16a34a"
            ...
            <details style="margin-bottom:7px;border-radius:10px;overflow:hidden;
                            border:1px solid {_pos_border};box-shadow:0 1px 4px rgba(0,0,0,0.08);">
              <summary style="cursor:pointer;padding:9px 13px;background:{_pos_hdr_bg};
                ...
                POZİTİF ETKENLER
                <span style="margin-left:6px;background:{_pos_border};color:#fff;
                             font-size:0.6rem;font-weight:800;padding:1px 6px;
                             border-radius:999px;">{len(score_pros)}</span>
            ...
            neg_items_html = ""
            if score_cons:
                _n_txt  = "#fecaca" if _dark else "#7f1d1d"
                _n_line = "rgba(239,68,68,0.15)" if _dark else "rgba(220,38,38,0.15)"
                for c in score_cons:
                    neg_items_html += (
                        f"<div style='display:flex;align-items:flex-start;gap:6px;"
                        f"padding:5px 0;border-bottom:1px solid {_n_line};'>"
                        f"<span style='color:#ef4444;font-size:0.65rem;margin-top:2px;flex-shrink:0;'>●</span>"
                        f"<span style='font-size:0.7rem;line-height:1.4;color:{_n_txt};'>{c}</span>"
                        f"</div>"
                    )
            else:
                _n_txt = "#fca5a5" if _dark else "#6b7280"
            ...
            if _dark:
                _neg_hdr_bg   = "linear-gradient(90deg,#2d0a0a 0%,#1c0606 100%)"
                _neg_hdr_txt  = "#fca5a5"
                _neg_cnt_bg   = "rgba(45,10,10,0.55)"
                _neg_border   = "#ef4444"
            else:
                _neg_hdr_bg   = "linear-gradient(90deg,#fee2e2 0%,#fef2f2 100%)"
                _neg_hdr_txt  = "#991b1b"
                _neg_cnt_bg   = "#fef2f2"
                _neg_border   = "#dc2626"
            ...
            st.markdown(f"""
            <details style="margin-bottom:7px;border-radius:10px;overflow:hidden;
                            border:1px solid {_neg_border};box-shadow:0 1px 4px rgba(0,0,0,0.08);">
              <summary style="cursor:pointer;padding:9px 13px;background:{_neg_hdr_bg};
                ...
                <span style="font-size:1rem;line-height:1;">❌</span>
                NEGATİF ETKENLER
                <span style="margin-left:6px;background:{_neg_border};color:#fff;
                             font-size:0.6rem;font-weight:800;padding:1px 6px;
                             border-radius:999px;">{len(score_cons)}</span>
                <span style="margin-left:auto;font-size:0.65rem;opacity:0.5;">▾</span>
              </summary>
              <div class="custom-scroll"
    ...
    for i, ticker in enumerate(valid_tickers):
        try:
            ...
            if index_close is not None and len(index_close) > 10:
                ...
                if rsi_val > 45: is_powerful = True
...
with col_btn:
    ...
    if st.button("🕵️ TÜM PİYASAYI TARA (MASTER SCAN)", type="primary", use_container_width=True):
        ...
        try:
            ...
            # 10. FORMASYON - %75
            my_bar.progress(75, text="🦁Formasyonlar Taranıyor...%75")
            ...
            my_bar.progress(95, text="🔄 Güçlü Dönüş Adayları (RSI Diverjans + Birikim) Taranıyor...%95")
...
# --- PERSONA SEÇİMİ (Senaryo bazlı dinamik kimlik) ---
    # Öncelik sırası: Royal Flush > Z-Score aşırılık > Formasyon > Nötr
    try:
        _z = z_score_val
    except:
        _z = 0.0
...
*** 7. İSTATİSTİKSEL YÖNELIM (LORANTZİAN ML ANALİZİ) ***
...
EĞER Royal Flush sinyali tetiklendiyse:
...
""", unsafe_allow_html=True)
    ...
    for _k in ['ict_scan_data','guclu_donus_data','golden_pattern_data','harmonic_data',
                'harmonic_confluence_data','rs_leaders_data','accum_data','breakout_left',
                'breakout_right','pattern_data','minervini_data','golden_results','royal_results']:
        if _k not in st.session_state: st.session_state[_k] = None
    for _k in ['stp_scanned']:
        if _k not in st.session_state: st.session_state[_k] = False
    for _k in ['stp_crosses','stp_trends','stp_filtered']:
    ...
    with _c_left:
        ...
        _dark  = st.session_state.get('dark_mode', False)
        _cbg   = "#1e0f35" if _dark else "#faf5ff"
        if _hits:
            st.markdown(f"<div style='background:{_cbg};border:1px solid #7c3aed;border-radius:7px;"
                        f"padding:7px 10px;margin-bottom:5px;'>"
                        f"<span style='font-weight:800;color:#c084fc;font-size:0.83rem;'>"
                        f"🔥 CONFLUENCE — {len(_hits)} Hisse</span></div>", unsafe_allow_html=True)
            with st.container(height=160, border=False):
                for _ci, _ch in enumerate(_hits[:8]):
                    _cs = _ch['Sembol'].replace('.IS','')
                    _gc = _ch['group_count']
                    _ts = _ch['total_scanners']
                    _cp = _ch.get('price', 0)
                    _cp_s = f"{int(_cp)}" if _cp >= 1000 else f"{_cp:.2f}"
                    _badge = "🔥 3/3" if _gc == 3 else "⚡ 2/3"
                    if st.button(f"{_badge} {_cs} ({_cp_s}) — {_ts} yöntem",
                                 key=f"conf_top_{_cs}_{_ci}", use_container_width=True):
                        st.session_state.ticker = _ch['Sembol']
                        on_scan_result_click(_ch['Sembol']); st.rerun()
        else:
            st.markdown(f"<div style='background:{_cbg};border:1px dashed #7c3aed;border-radius:7px;"
                        f"padding:18px 10px;text-align:center;color:#94a3b8;font-size:0.8rem;'>"
                        f"🔥 CONFLUENCE<br><span style='font-size:0.7rem;'>Master Scan çalıştırın</span></div>",
                        unsafe_allow_html=True)
        _hc_df = st.session_state.get('harmonic_confluence_data')
        if _hc_df is not None and not (hasattr(_hc_df, 'empty') and _hc_df.empty):
            st.markdown("<div style='background:linear-gradient(90deg,#2d1060,#1e0f3a);border:2px solid #7c3aed;"
                        "border-radius:6px;padding:6px 10px;margin-top:5px;'>"
                        "<span style='font-size:0.78rem;font-weight:900;color:#c084fc;'>"
                        f"⚡ HARMONİK CONF ({len(_hc_df)} hisse)</span></div>", unsafe_allow_html=True)
            for _hci, _hcr in _hc_df.head(4).iterrows():
                _hcs = str(_hcr.get('Sembol','')).replace('.IS','')
                _hcp = _hcr.get('Fiyat', 0)
                _hcp_s = f"{int(_hcp)}" if _hcp >= 1000 else f"{_hcp:.2f}"
                if st.button(f"⚡ {_hcs} ({_hcp_s})", key=f"hctop_{_hcs}_{_hci}", use_container_width=True):
    ...
    with _c_right:
        st.markdown(_scan_card_header("💎", "ELİTLER", 88,
            "Platin Fırsat (5 kriter) + Altın Fırsat (3 kriter) — piyasanın kremi",
        ...
        if st.button("💎 ELİT TARAMA (Platin + Altın)", use_container_width=True, key="btn_elit_tara_main",
            ...
            with st.spinner("Elit hisseler aranıyor..."):
                ...
                if _scan_list:
                    ...
                    st.session_state.golden_results = _df_g.sort_values(by="M.Cap", ascending=False).reset_index(drop=True) if not _df_g.empty else pd.DataFrame()
                    st.session_state.royal_results  = _df_r.sort_values(by="M.Cap", ascending=False).reset_index(drop=True) if not _df_r.empty else pd.DataFrame()
                    st.rerun()
        _has_elite = (st.session_state.royal_results is not None and not st.session_state.royal_results.empty) or \
        ...
        if _has_elite:
            with st.container(height=190, border=False):
                if st.session_state.royal_results is not None and not st.session_state.royal_results.empty:
                    st.markdown("<div style='font-size:0.76rem;font-weight:800;color:#3b82f6;margin-bottom:4px;'>💎 PLATİN FIRSAT</div>", unsafe_allow_html=True)
                    for _ri, _rr in st.session_state.royal_results.head(4).iterrows():
                        _rs = _rr['Hisse']; _rd = get_display_name(_rs)
                        _rfv = _rr['Fiyat']; _rfs = f"{int(_rfv)}" if _rfv >= 1000 else f"{_rfv:.2f}"
                        if st.button(f"💎 {_rd} ({_rfs})", key=f"elit_royal_{_ri}", use_container_width=True):
                            on_scan_result_click(_rs); st.rerun()
                if st.session_state.golden_results is not None and not st.session_state.golden_results.empty:
                    st.markdown("<div style='font-size:0.76rem;font-weight:800;color:#d97706;margin-bottom:4px;margin-top:5px;'>🦁 ALTIN FIRSAT</div>", unsafe_allow_html=True)
                    for _gi, _gr in st.session_state.golden_results.head(4).iterrows():
                        _gs = _gr['Hisse']; _gd = get_display_name(_gs)
                        _gfv = _gr['Fiyat']; _gfs = f"{int(_gfv)}" if _gfv >= 1000 else f"{_gfv:.2f}"
                        if st.button(f"🦁 {_gd} ({_gfs})", key=f"elit_gold_{_gi}", use_container_width=True):
                            on_scan_result_click(_gs); st.rerun()
        else:
    ...
    with _t1c1:
        st.markdown(_scan_card_header("🦅", "ICT Sniper", 85, "MSS + FVG + OTE · Kurumsal ayak izi", "#16a34a"), unsafe_allow_html=True)
        if st.button(f"🦅 ICT SNIPER TARA ({st.session_state.category})", type="secondary", use_container_width=True, key="btn_scan_ict",
                     help="Kurumsal yatırımcıların (bankalar, fonlar) piyasaya giriş izlerini takip eder.\n\nSırasıyla 5 koşul aranır: 1) Likidite süpürmesi (stop avı), 2) Piyasa yapısı kırılımı (MSS), 3) Güçlü itme hareketi (1.3× normal hacim), 4) Boşluk bölgesi (FVG) oluşumu, 5) Risk/Ödül oranı en az 2.5×.\n\nTüm koşullar sırayla gerçekleşmişse bu bir kurumsal kurulum sayılır. Geçmiş veriye değil, son 50 bara bakar."):
    ...
    if st.session_state.ict_scan_data is not None:
        ...
        if not df_res.empty:
            ...
            with _ic1:
                st.markdown(f"<div style='text-align:center;color:#16a34a;font-weight:800;font-size:0.75rem;padding:3px;border-radius:4px;border:1px solid #86efac;margin-bottom:5px;'>🐂 LONG ({len(longs)})</div>", unsafe_allow_html=True)
                with st.container(height=120):
                    if not longs.empty:
                        for i, row in longs.iterrows():
                            sym = row['Sembol']
                            if st.button(f"🐂 {sym.replace('.IS','')} ({row['Fiyat']:.2f}) | {row['Durum']}", key=f"ict_long_{sym}_{i}", use_container_width=True, help=f"Stop Loss: {row['Stop_Loss']}"):
                                on_scan_result_click(sym); st.rerun()
                    else:
                        st.caption("Long yok.")
            with _ic2:
                st.markdown(f"<div style='text-align:center;color:#dc2626;font-weight:800;font-size:0.75rem;padding:3px;border-radius:4px;border:1px solid #fca5a5;margin-bottom:5px;'>🐻 SHORT ({len(shorts)})</div>", unsafe_allow_html=True)
                with st.container(height=120):
                    if not shorts.empty:
                        for i, row in shorts.iterrows():
                            sym = row['Sembol']
                            if st.button(f"🐻 {sym.replace('.IS','')} ({row['Fiyat']:.2f}) | {row['Durum']}", key=f"ict_short_{sym}_{i}", use_container_width=True, help=f"Stop Loss: {row['Stop_Loss']}"):
                                on_scan_result_click(sym); st.rerun()
                    else:
                        st.caption("Short yok.")
        else:
            st.info("High Probability ICT kurulumu tespit edilemedi.")
    ...
    st.markdown("<div style='border-left:5px solid #3b82f6;padding-left:10px;margin-bottom:8px;"
                "font-weight:900;font-size:1rem;color:#3b82f6;'>🥈 TIER 2 — Yüksek Güven</div>",
                unsafe_allow_html=True)
    ...
    with _t2c2:
        ...
        if st.session_state.stp_scanned or (st.session_state.accum_data is not None):
            ...
            with _s1:
                st.markdown("<div style='text-align:center;color:#1e40af;font-weight:700;font-size:0.7rem;margin-bottom:3px;'>⚡ STP KESİŞİM</div>", unsafe_allow_html=True)
                with st.container(height=120, border=True):
                    if st.session_state.stp_crosses:
                        for item in st.session_state.stp_crosses:
                            if st.button(f"🚀 {item['Sembol']}", key=f"stp_c_{item['Sembol']}", use_container_width=True):
                                st.session_state.ticker = item['Sembol']; st.rerun()
                    else:
                        st.caption("Kesişim yok.")
            with _s2:
                st.markdown("<div style='text-align:center;color:#7c3aed;font-weight:700;font-size:0.7rem;margin-bottom:3px;'>🤫 AKILLI PARA</div>", unsafe_allow_html=True)
                with st.container(height=120, border=True):
                    if st.session_state.accum_data is not None and not st.session_state.accum_data.empty:
                        for _idx, _arow in st.session_state.accum_data.iterrows():
                            _aicon = "⚡" if _arow.get('Pocket_Pivot', False) else "🎩"
                            _aq = "💎" if "A KALİTE" in _arow.get('Kalite','') else ""
                            if st.button(f"{_aicon}{_aq} {_arow['Sembol'].replace('.IS','')}", key=f"btn_acc_{_arow['Sembol']}_{_idx}", use_container_width=True):
                                on_scan_result_click(_arow['Sembol']); st.rerun()
                    else:
            ...
            if not _formations.empty:
                with st.container(height=150, border=True):
                    for i, row in _formations.head(12).iterrows():
                        sym = row['Sembol']
                        prefix = "♠️" if row.get('is_royal', False) else "🚀"
    ...
    with _t2c4:
        st.markdown(_scan_card_header("⚡", "Harmonik Confluence", 90, "Fibonacci PRZ + ICT Zone + RSI Div", "#7c3aed"), unsafe_allow_html=True)
        if st.button(f"⚡ HARMONİK CONFLUENCE TARA ({st.session_state.category})", type="secondary", use_container_width=True, key="btn_scan_harmonic_conf",
            ...
            with st.spinner("3 metodoloji kesişiyor: Fibonacci + ICT + RSI Diverjansı..."):
                current_assets = ASSET_GROUPS.get(st.session_state.category, [])
                st.session_state.harmonic_confluence_data = scan_harmonic_confluence_batch(current_assets)
                st.rerun()
        _hc_scan = st.session_state.harmonic_confluence_data
        if _hc_scan is not None and hasattr(_hc_scan, 'empty') and not _hc_scan.empty:
            with st.container(height=150, border=True):
                for _hci, _hcr in _hc_scan.iterrows():
                    _hcs = str(_hcr.get('Sembol','')).replace('.IS','')
                    _hcp = _hcr.get('Fiyat', 0)
                    _hcp_s = f"{int(_hcp)}" if _hcp >= 1000 else f"{_hcp:.2f}"
                    _cyon = str(_hcr.get('Yön',''))
                    _cyon_lbl = "🟢" if ('Bullish' in _cyon or 'LONG' in _cyon) else "🔴"
                    if st.button(f"⚡{_cyon_lbl} {_hcs} ({_hcp_s}) | {_hcr.get('Pattern','')}", key=f"hconf_scan_{_hcs}_{_hci}", use_container_width=True):
                        st.session_state.ticker = _hcr.get('Sembol', _hcs)
                        on_scan_result_click(_hcr.get('Sembol', _hcs)); st.rerun()
    ...
    # ══════════════════════════════════════════════════════════
    # ④ TIER 3 — Destekleyici Taramalar (Expander)
    # ══════════════════════════════════════════════════════════
    with st.expander("🥉 TIER 3 — Destekleyici Taramalar (Breakout · Formasyon · Harmonik · RS Liderleri)"):
        ...
        with _t3c1:
            ...
            if st.button(f"⚡ BREAKOUT TARA ({st.session_state.category})", type="secondary", key="dual_breakout_btn", use_container_width=True,
                ...
                with st.spinner("Hem ısınanlar hem kıranlar taranıyor..."):
                    ...
                    st.session_state.breakout_right = scan_confirmed_breakouts(curr_list, st.session_state.get('category', 'S&P 500'))
                    st.rerun()
            if st.session_state.breakout_left is not None or st.session_state.breakout_right is not None:
                _bc1, _bc2 = st.columns(2)
                with _bc1:
                    st.markdown("<div style='text-align:center;color:#d97706;font-weight:700;font-size:0.7rem;margin-bottom:3px;'>🔥 ISINANLAR</div>", unsafe_allow_html=True)
                    with st.container(height=120):
                        if st.session_state.breakout_left is not None and not st.session_state.breakout_left.empty:
                            for i, (_, row) in enumerate(st.session_state.breakout_left.head(12).iterrows()):
                                sym_raw = row.get("Sembol_Raw", row.get("Sembol", "UNK"))
                                prox_clean = str(row['Zirveye Yakınlık']).split('<')[0].strip()
                                if st.button(f"🔥 {sym_raw.replace('.IS','')} | {prox_clean}", key=f"L_btn_new_{sym_raw}_{i}", use_container_width=True):
                                    on_scan_result_click(sym_raw); st.rerun()
                        else:
                            st.caption("Yok.")
                with _bc2:
                    st.markdown("<div style='text-align:center;color:#16a34a;font-weight:700;font-size:0.7rem;margin-bottom:3px;'>🔨 KIRANLAR</div>", unsafe_allow_html=True)
                    with st.container(height=120):
                        if st.session_state.breakout_right is not None and not st.session_state.breakout_right.empty:
                            for i, (_, row) in enumerate(st.session_state.breakout_right.head(12).iterrows()):
                                sym = row['Sembol']
                                if st.button(f"🚀 {sym.replace('.IS','')} ({row['Fiyat']}) | {row['Hacim_Kati']}", key=f"R_btn_new_{sym}_{i}", use_container_width=True):
                                    on_scan_result_click(sym); st.rerun()
                        else:
                            st.caption("Yok.")
        ...
        with _t3c2:
            ...
            if st.session_state.pattern_data is not None:
                if len(st.session_state.pattern_data) > 0:
                    with st.container(height=150, border=True):
                        for i, row in st.session_state.pattern_data.iterrows():
                            sym = row['Sembol']
                            pat = row['Formasyon']
                            icon = "🚩" if "BAYRAK" in pat else "📦" if "RANGE" in pat else "🧛"
                            if st.button(f"{icon} {sym.replace('.IS','')} ({row['Fiyat']:.2f}) | {pat}", key=f"pat_{sym}_{i}", use_container_width=True, help=row['Detay']):
                                on_scan_result_click(sym); st.rerun()
                else:
                    st.info("Kitabi formasyon bulunamadı.")
        ...
        with _t3c3:
            ...
            if st.button(f"🔮 HARMONİK TARA ({st.session_state.category})", type="secondary", use_container_width=True, key="btn_scan_harmonic",
                ...
                with st.spinner("Gartley, Butterfly, Bat, Crab, Shark — PRZ bölgeleri aranıyor..."):
                    current_assets = ASSET_GROUPS.get(st.session_state.category, [])
                    st.session_state.harmonic_data = scan_harmonic_patterns_batch(current_assets)
                    st.rerun()
            if st.session_state.harmonic_data is not None:
                _hm = st.session_state.harmonic_data
                if not _hm.empty:
                    with st.container(height=150, border=True):
                        for i, row in _hm.iterrows():
                            sym = row['Sembol']
                            durum = row.get('Durum','')
                            d_txt = "D:bekleniyor" if durum == '📍 Yaklaşıyor' else f"D:{row['Bar_Önce']}g"
                            if st.button(f"{durum} {row['Pattern']} | {row['Yön']} | {sym.replace('.IS','')} PRZ:{row['PRZ']} %{row['PRZ_Fark%']} | {d_txt}", key=f"harm_btn_{sym}_{i}", use_container_width=True):
                                on_scan_result_click(sym); st.rerun()
                else:
        ...
        with _t3c4:
            ...
            if st.session_state.rs_leaders_data is not None:
                if len(st.session_state.rs_leaders_data) > 0:
                    with st.container(height=150, border=True):
                        for i, row in st.session_state.rs_leaders_data.iterrows():
                            sym = row['Sembol']
                            alpha_5 = row['Alpha_5D']
                            icon = "🔥" if alpha_5 > 5.0 else "💪"
                            if st.button(f"{icon} {sym.replace('.IS','')} ({row['Fiyat']:.2f}) | Alpha:+%{alpha_5:.1f} | Vol:{row['Hacim_Kat']:.1f}x", key=f"rs_lead_{sym}_{i}", use_container_width=True):
                                on_scan_result_click(sym); st.rerun()
                else:
    ...
    cursor:pointer !important; position:absolute; top:0; left:0;
            ...
            with st.container(height=500, border=True):
                for i, item in enumerate(st.session_state.top_20_summary):
                    ...
                    _dark = st.session_state.get('dark_mode', False)
                    if score >= 80:
                        bg_color = "#2d2007" if _dark else "#fffbeb"; border_color = "#f59e0b"
                        score_bg = "linear-gradient(90deg, #f59e0b 0%, #d97706 100%)"
                    elif score >= 50:
                        bg_color = "#0f1e35" if _dark else "#f8fafc"; border_color = "#3b82f6"
                        score_bg = "#3b82f6"
                    else:
                        bg_color = "#1a1f2e" if _dark else "#ffffff"; border_color = "#475569"
                        score_bg = "#64748b"
                    ...
                    onay_badge = (f'<span style="background:linear-gradient(90deg,#7c3aed,#4f46e5);color:white;padding:2px 8px;border-radius:10px;font-size:0.72rem;font-weight:800;margin-left:6px;">🎖️ {onay_sayisi} Onay</span>'
                                  ...
                                  f'<span style="background:#f1f5f9;color:#1e293b;padding:2px 8px;border-radius:10px;font-size:0.72rem;font-weight:700;margin-left:6px;">✅ {onay_sayisi} Onay</span>')
                    st.markdown(f"""
<div class="t20wrap">
